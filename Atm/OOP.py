...
obj = Child()
obj.Life_Indication() # AttributeError: 'Child' object has no attribute 'Age'
...
obj = Child()
obj.Life_Indication()  # INDICATION: 22  # INDICATION: Ahsan


#:::::::::::::::::::::::::::::::super().__init__(self), super keeps inheritance and we do not need to write
...
def first_class_method(arguments):
    return f'Args: {arguments}'

def res(first_class_method):
    return first_class_method

print(   res(    first_class_method('data')  )    )
...
# Python Uses MRO , Method Resolution Order , MRO uses C3 Literalization that creates
# a specific order of classes when looking up for methods.
...
# -----> Not possible :- ,Rule:- variable must be declared global before its first use inside the function.
A = 100
print(A)
...
def outer():
    x = 10  # if here we dont set x then: SyntaxError: no binding for nonlocal 'x' found (Rule: should be present in outer function )
    print(f'x before inner: {x}') # 10

    def inner():
        nonlocal x
        x = 25
        print(f'x in inner: {x}') # 25
    inner()

    print(f'x after inner: {x}') # 25
...
#
# Map & Strip Function:---
# map function is used to apply a function to a iterable data (list or tuple) and returns a map object , which is a iteratable.
# --> map(function, data(iterable) )
#
...
res = eval(expression, {'__builtins__': None}, local_dict,
           global_dict)  # TypeError: eval expected at most 3 arguments, got 4
...
#------------------: # Note: If data is not a string, it raises the TypeError exception.
data = [1,2,3]
res = ''.join(data)
print(res)  # TypeError: sequence item 0: expected str instance, int found
